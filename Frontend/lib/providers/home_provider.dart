...
class HomeProvider extends ProviderBase {
  ...
  HomeProvider(this._homeService, SharedPreferences sharedPreferences)
  ...
  Future<void> getCaffList() async {
    ...
    if (token != null) {
      ApiResult? apiResult =
          await _homeService.getCaffList(token, searchController.text);
      if (apiResult != null) {
        if (apiResult.isSuccess) {
          ...
          for (var c in apiResult.data) {
            ...
            temp.imgURL =
                "${Globals.baseIp}/${temp.imgURL.split('\\').join('/')}";
            caffList.add(temp);
          }
          notifyListeners();
        } else {
